  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = fishbot_interfaces::srv::FishBotConfig_Request;
    is_plain =
      (
...
static size_t _FishBotConfig_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_FishBotConfig_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = fishbot_interfaces::srv::FishBotConfig_Response;
    is_plain =
      (
      offsetof(DataType, value) +
      last_member_size
...
static size_t _FishBotConfig_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_FishBotConfig_Response(full_bounded, is_plain, 0);
