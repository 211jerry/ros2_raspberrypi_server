...
class SweeperNode(BasicNavigator):
    def __init__(self, node_name='sweeper_node'):
        ...
        self.get_logger().info(f'路径优化完成，最终生成 {len(self.waypoints)} 个连通路径点（含返回原点）')
    ...
    def find_best_next_strip(self, current_endpoint_pix: tuple, unvisited_strips: list, binary_map: np.ndarray) -> tuple:
        ...
        # 1. 优先按最小距离排序（核心：确保最近的条带优先）
        strip_candidates.sort(key=lambda x: x['min_dist'])
        
        # 2. 遍历候选条带，优先选择距离当前端点更近的端点（而非仅看安全）
        for candidate in strip_candidates:
            ep1_dist = candidate['ep1']['dist']
            ep2_dist = candidate['ep2']['dist']
            # 优先选距离更近的端点，而非先看安全
            if ep1_dist < ep2_dist:
                if candidate['ep1']['safe']:
                    return candidate['strip'], candidate['ep1']['pix'], True
                else:
                    # 即使不安全，也选更近的端点（后续走A*）
                    return candidate['strip'], candidate['ep1']['pix'], False
            else:
                if candidate['ep2']['safe']:
                    return candidate['strip'], candidate['ep2']['pix'], True
                else:
                    return candidate['strip'], candidate['ep2']['pix'], False
    ...
    def generate_strips_for_region(self, binary_map: np.ndarray, region_mask: np.ndarray, bbox: tuple) -> list:
        ...
        for strip_idx, strip_y_start in enumerate(range(y_min, y_max, strip_width_pix)):
            ...
            for interval in continuous_intervals:
                if len(interval) < 2:  # 至少需要2个像素构成条带
                    continue
                ...
                start_yaw = 0.0 if is_even_strip else math.pi
                end_yaw = math.pi if is_even_strip else 0.0
                
                # 条带起点（像素坐标）
                start_pix = (start_x, y_pix)
                # 条带终点（像素坐标）
                end_pix = (end_x, y_pix)
                
                # 校验起点/终点是否在自由空间
                if (start_pix[0] < 0 or start_pix[0] >= self.map_w or start_pix[1] < 0 or start_pix[1] >= self.map_h or
                    binary_map[start_pix[1], start_pix[0]] != 1):
                    continue
                if (end_pix[0] < 0 or end_pix[0] >= self.map_w or end_pix[1] < 0 or end_pix[1] >= self.map_h or
                    binary_map[end_pix[1], end_pix[0]] != 1):
                    continue
                
                # 转换为世界坐标
                start_world_x, start_world_y = self.pix2world(start_x, y_pix)
                end_world_x, end_world_y = self.pix2world(end_x, y_pix)
                
                # 条带的路径点列表（仅两端）
                strip_waypoints_pix = [(start_x, y_pix, start_yaw), (end_x, y_pix, end_yaw)]
                strip_waypoints_world = [(start_world_x, start_world_y, start_yaw), (end_world_x, end_world_y, end_yaw)]
                
                strips.append((start_pix, end_pix, strip_waypoints_pix, strip_waypoints_world))
        
        self.get_logger().info(f'单连通域生成 {len(strips)} 个有效条带（仅保留两端点）')
        return strips
    ...
    def draw_path_on_map(self, waypoints):
        ...
        COLOR_START = (0, 255, 0)  # 起点（原点）颜色
        COLOR_END = (255, 0, 255)  # 终点（原点）颜色（区分中间点）
        ...
        if waypoints:
            ...
            if self.draw_waypoints:
                ...
                for idx, (x_pix, y_pix) in enumerate(pix_waypoints):
                    if idx == 0:
                        # 起点（原点）用绿色大圆点
                        cv2.circle(draw_img, (x_pix, y_pix), 6, COLOR_START, -1)
                    elif idx == len(pix_waypoints)-1:
                        # 终点（原点）用紫色大圆点（与起点区分）
                        cv2.circle(draw_img, (x_pix, y_pix), 6, COLOR_END, -1)
                    elif idx % step == 0 and 0 <= x_pix < w and 0 <= y_pix < h:
    ...
    def generate_coverage_path(self):
        ...
        start_strip = None
        start_ep = None  # 记录初始位置对应的条带端点
        step_pix = max(1, int(self.step_size / self.resolution))
        
        # 查找初始位置所在的条带（匹配最近的端点）
        for strip in all_strips:
            ep1_pix, ep2_pix, strip_pix, strip_world = strip
            dist1 = math.hypot(ep1_pix[0] - start_pix[0], ep1_pix[1] - start_pix[1])
            dist2 = math.hypot(ep2_pix[0] - start_pix[0], ep2_pix[1] - start_pix[1])
            min_dist = min(dist1, dist2)
            if min_dist <= step_pix * 1.5:
                start_strip = strip
                start_ep = ep1_pix if dist1 < dist2 else ep2_pix
                break
        
        global_waypoints = []
        final_endpoint_pix = None  # 记录最后一个条带的端点（用于连接原点）
        if start_strip is None:
            ...
            final_endpoint_pix = current_endpoint_pix
        else:
            self.get_logger().info(f'初始位置位于条带端点附近: {start_ep}')
            unvisited_strips = [s for s in all_strips if s != start_strip]
            ep1_pix, ep2_pix, strip_pix, strip_world = start_strip
            
            # 初始条带仅添加两端点（按初始位置最近的端点排序）
            if start_ep == ep1_pix:
                global_waypoints.append(strip_world[0])
                global_waypoints.append(strip_world[1])
                current_endpoint_pix = ep2_pix
            else:
                global_waypoints.append(strip_world[1])
                global_waypoints.append((strip_world[0][0], strip_world[0][1], strip_world[0][2] + math.pi))
                current_endpoint_pix = ep1_pix
            ...
            while unvisited_strips:
                ...
                if next_start_ep_pix == next_ep1_pix:
                    ...
                    reversed_world = [
                        (next_strip_world[1][0], next_strip_world[1][1], next_strip_world[1][2]),
                        (next_strip_world[0][0], next_strip_world[0][1], next_strip_world[0][2] + math.pi)
                    ]
            ...
            final_endpoint_pix = current_endpoint_pix  # 记录清扫完成后的端点
        
        # ===================== 【核心新增】添加返回原点逻辑 =====================
        if final_endpoint_pix is not None:
            origin_world = (self.init_x, self.init_y)  # 原点世界坐标
            origin_pix = self.world2pix(origin_world[0], origin_world[1])  # 原点像素坐标
            
            self.get_logger().info(f'清扫完成，准备返回原点 ({origin_world[0]}, {origin_world[1]})')
            
            # 校验最后一个端点到原点的连通性
            if self.is_line_safe(binary, final_endpoint_pix, origin_pix):
                # 连通：直接添加原点作为目标点
                dx = origin_world[0] - self.pix2world(*final_endpoint_pix)[0]
                dy = origin_world[1] - self.pix2world(*final_endpoint_pix)[1]
                transition_yaw = math.atan2(dy, dx)
                # 添加过渡点（确保路径平滑）
                global_waypoints.append((origin_world[0], origin_world[1], transition_yaw))
            else:
                # 不连通：用A*生成返回路径
                self.get_logger().warn(f'最后一个端点到原点不连通，生成A*返回路径')
                connect_path_pix = self.a_star_search(binary, final_endpoint_pix, origin_pix)
                if connect_path_pix:
                    # 按步长采样返回路径点
                    for i in range(1, len(connect_path_pix), step_pix):
                        px, py = connect_path_pix[i]
                        ...
                        global_waypoints.append((wx, wy, yaw))
                    # 添加原点作为最终目标点
                    global_waypoints.append((origin_world[0], origin_world[1], self.init_yaw))
                else:
                    self.get_logger().error(f'无法生成返回原点的路径，跳过返回逻辑')
    ...
    def nav_to_pose(self, target_pose, index, total):
        # 标记原点目标点
        is_origin = (abs(target_pose.pose.position.x - self.init_x) < 0.01) and (abs(target_pose.pose.position.y - self.init_y) < 0.01)
        if is_origin:
            self.get_logger().info(f'[{index}/{total}] 前往原点目标点: ({target_pose.pose.position.x:.2f}m, {target_pose.pose.position.y:.2f}m)')
        else:
    ...
    def run(self):
        try:
            ...
            self.get_logger().info(f'开始全覆盖清扫，共 {total} 个连通路径点（含返回原点）')
            success_count = 0
            ...
            self.get_logger().info(f'清扫完成！成功到达 {success_count}/{total} 个路径点（含返回原点）')
        except Exception as e:
